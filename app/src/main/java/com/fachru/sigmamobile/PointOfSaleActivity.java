...
//        custid = intent.getLongExtra(CustomerActivity.CUSTID, -1);
		custid = intent.getLongExtra(OutletActivity.OUTLET_ID, -1);
...
//                bundle.putLong(CustomerActivity.CUSTID, custid);
				bundle.putLong(OutletActivity.OUTLET_ID, custid);
	...
	private void actionPrint(String title) {
		...
		try {
			...
			for (DoItem item : doHead.doItems()) {
				...
				table.addCell(createCell(CommonUtil.percentFormat(item.discountPrinc + item.discountNst), font, Element.ALIGN_RIGHT, Rectangle.NO_BORDER, 1));
